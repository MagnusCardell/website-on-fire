...
export const Card = forwardRef<HTMLDivElement, CardProps>(({
  ...
  if (!card.faceUp) {
    return (
      <div
        ...
        style={{ width: 'var(--sol-card-w)', height: 'var(--sol-card-h)', ...style, transformStyle: 'preserve-3d',
        }}
      >
  ...
  return (
    ...
    >
      ...
      {/* Center suit */}
      <div className={cn('absolute inset-0 flex items-center justify-center text-xl', colorClass)}>
        {symbol}
      </div>
      
      {/* Bottom right rank and suit (inverted) */}
      <div className={cn('absolute bottom-0.5 right-1.5 leading-none rotate-180', colorClass)}>
        <div className='text-sm font-bold'>{card.rank}</div>
      </div>
